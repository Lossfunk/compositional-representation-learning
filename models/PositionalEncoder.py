...
class PositionalEncoder(nn.Module):
    ...
    def forward(self, coordinates: torch.Tensor): # (batch_size, N, 2)
        y_embeddings = self.generate_sinusoidal_1d(coordinates[:, :, 0]) # (batch_size, N, half_embed_dim)
        x_embeddings = self.generate_sinusoidal_1d(coordinates[:, :, 1]) # (batch_size, N, half_embed_dim)
