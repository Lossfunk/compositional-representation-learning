...
class HyperNetworkSpatialEncoder(L.LightningModule):
    ...
    def forward(self, x):
        ...
        for datapoint_idx in range(batch_size):
            ...
            object_coordinates_feat_y = (object_coordinates_img[:, 0].float() * scale_factor_y).floor().long() # (N,)
            object_coordinates_feat_x = (object_coordinates_img[:, 1].float() * scale_factor_x).floor().long() # (N,)
            
            object_coordinates_feat = torch.stack([object_coordinates_feat_y, object_coordinates_feat_x], dim=1) # (N, 2)
            ...
            object_feat_top_left = torch.stack([object_feat_y_min, object_feat_x_min], dim=1) # (1, 2)

            object_coordinates_feat_relative = object_coordinates_feat - object_feat_top_left # (M, 2)
            object_positional_embeddings = self.positional_encoder(object_coordinates_feat_relative.unsqueeze(0).float()) # (1, M, embed_dim)

            spatial_operator_embedding = self.permutation_invariant_encoder(object_positional_embeddings).squeeze(0) # (embed_dim,)
            spatial_operator_embeddings.append(spatial_operator_embedding)

            object_features = feature_map[:, object_coordinates_feat[:, 0], object_coordinates_feat[:, 1]].permute(1, 0) # (M, embed_dim)
            object_features = object_features.unsqueeze(0) + object_positional_embeddings # (1, M, embed_dim)
