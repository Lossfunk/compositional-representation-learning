...
class PESpatialEncoder(L.LightningModule):
    def __init__(self, config):
        ...
        self.project_positional_embeddings = config["model"]["config"][
            "project_positional_embeddings"
        ]
    ...
    def forward(self, x):
        ...
        for datapoint_idx in range(batch_size):
            ...
            object_mask_featmap = F.interpolate(
                object_mask_img,
            ...
            object_coordinates_featmap = (object_mask_featmap >= 0.5).nonzero(
                as_tuple=False
            )  # (N, 2)
            object_coordinates_y_min = object_coordinates_featmap[:, 0].min()
            object_coordinates_x_min = object_coordinates_featmap[:, 1].min()
            object_coordinates_top_left = torch.stack(
                [object_coordinates_y_min, object_coordinates_x_min]
            ).unsqueeze(
                0
            )  # (1, 2)
            object_coordinates_relative = (
                object_coordinates_featmap - object_coordinates_top_left
            )  # (N, 2)

            object_positional_embeddings = self.positional_encoder(
                object_coordinates_relative.unsqueeze(0).float()
            )  # (1, N, embed_dim)
            if self.project_positional_embeddings:
                object_positional_embeddings = self.project_to_negative_hypersphere(
                    object_positional_embeddings.squeeze(0)
                ).unsqueeze(
                    0
                )  # (1, N, embed_dim)

            object_features = feature_maps[datapoint_idx][
                :, object_coordinates_featmap[:, 0], object_coordinates_featmap[:, 1]
            ].permute(
                1, 0
            )  # (N, embed_dim)
            object_features = self.project_to_negative_hypersphere(
                object_features
            )  # (N, embed_dim)

            if self.position_composition_method == "add":
                object_lifted_features = (
                    object_features.unsqueeze(0) + object_positional_embeddings
                )  # (1, N, embed_dim)
            elif self.position_composition_method == "max":
                object_lifted_features = torch.maximum(
                    object_features.unsqueeze(0), object_positional_embeddings
                )  # (1, N, embed_dim)
            else:
                raise ValueError(
                    f"Invalid position composition method: {self.position_composition_method}"
                )

            computed_object_representation = self.permutation_invariant_encoder(
                object_lifted_features
            ).squeeze(
                0
            )  # (embed_dim,)
            ground_truth_object_representation = object_lifted_features.max(dim=1).values.squeeze(
                0
            )  # (embed_dim,)
        ...
        computed_object_representations = torch.stack(
            computed_object_representations
        )  # (batch_size, embed_dim)
        ground_truth_object_representations = torch.stack(
            ground_truth_object_representations
        )  # (batch_size, embed_dim)
    ...
    def project_to_negative_hypersphere(self, embeddings, eps=1e-8):
        ...
        projected_embeddings = F.normalize(
            negative_embeddings, p=2, dim=1
        )  # (batch_size, embed_dim)
