...
class ImageEncoder(nn.Module):
    def __init__(self, embed_dim):
        ...
        self.conv_2 = nn.Conv2d(
            in_channels=8, out_channels=8, kernel_size=3, stride=1, padding="same"
        )

        self.maxpool_1 = nn.MaxPool2d(kernel_size=2, stride=2)

        self.conv_3 = nn.Conv2d(
            in_channels=8, out_channels=12, kernel_size=3, stride=1, padding="same"
        )
        self.conv_4 = nn.Conv2d(
            in_channels=12, out_channels=12, kernel_size=3, stride=1, padding="same"
        )

        self.maxpool_2 = nn.MaxPool2d(kernel_size=2, stride=2)

        self.conv_5 = nn.Conv2d(
            in_channels=12, out_channels=20, kernel_size=3, stride=1, padding="same"
        )
        self.conv_6 = nn.Conv2d(
            in_channels=20, out_channels=20, kernel_size=3, stride=1, padding="same"
        )

        self.maxpool_3 = nn.MaxPool2d(kernel_size=2, stride=2)

        self.conv_7 = nn.Conv2d(
            in_channels=20, out_channels=32, kernel_size=3, stride=1, padding="same"
        )
        ...
        self.conv_10 = nn.Conv2d(
            in_channels=48, out_channels=48, kernel_size=1, stride=1, padding="same"
        )
        ...
        self.conv_14 = nn.Conv2d(
            in_channels=96, out_channels=96, kernel_size=1, stride=1, padding="same"
        )

        self.flatten = nn.Flatten()
        self.mlp = nn.Sequential(nn.Linear(96 * 3 * 3, 256), nn.ReLU(), nn.Linear(256, embed_dim))
...
class PatchBoxEmbeddings(L.LightningModule):
    def __init__(self, config):
        ...
        self.foundational_embeddings_distance_margin = config["model"]["config"][
            "foundational_embeddings_distance_margin"
        ]
        ...
        self.positional_encoder = PositionalEncoder(
            input_dim=2,
            output_dim=self.box_embed_dim,
            hidden_dim=self.hidden_dim,
            num_hidden_layers=self.num_hidden_layers,
        )
        self.binding_function = BindingFunction(
            input_dim=self.box_embed_dim * 2,
            output_dim=self.box_embed_dim,
            hidden_dim=self.hidden_dim,
            num_hidden_layers=self.num_hidden_layers,
        )
        ...
        self.vq = VectorQuantize(
            dim=self.box_embed_dim, **config["model"]["config"]["codebook_config"]
        )
    ...
    def forward(self, x):
        ...
        batch_image_patches = self.divide_image_into_patches(
            images
        )  # (batch_size, grid_h, grid_w, C, patch_h, patch_w)
        batch_mask_patches = self.divide_image_into_patches(
        ...
        codebook_positive_volume_loss = self.compute_positive_volume_loss(
            codebook_embeddings
        )  # scalar
        ...
        for datapoint_idx in range(batch_size):
            ...
            mask_patches = batch_mask_patches[
                datapoint_idx
            ]  # (grid_h, grid_w, 1, patch_h, patch_w)
            bounding_box = batch_bounding_boxes[datapoint_idx]  # (4,)

            # Get the patches that contain the object
            object_patches = image_patches[
                bounding_box[0] : bounding_box[2] + 1,
                bounding_box[1] : bounding_box[3] + 1,
            ]  # (bbox_grid_h, bbox_grid_w, C, patch_h, patch_w)
            object_region = self.stitch_object_patches(
                object_patches
            )  # (C, stitched_h, stitched_w)
            bbox_grid_h, bbox_grid_w, C, patch_h, patch_w = object_patches.shape
            num_patches = bbox_grid_h * bbox_grid_w
            object_patches_flattened = object_patches.reshape(
            ...
            object_patches_resized = F.interpolate(
                object_patches_flattened,
                size=(224, 224),
                mode="bilinear",
                align_corners=False,
            )  # (num_patches, C, 224, 224)
            object_region_resized = F.interpolate(
                object_region.unsqueeze(0),
                size=(224, 224),
                mode="bilinear",
                align_corners=False,
            )  # (1, C, 224, 224)
            ...
            object_grid_coords_normalized = self.compute_positional_data(
                object_patches
            )  # (num_patches, 2)
            object_positional_embeddings = self.positional_encoder(
                object_grid_coords_normalized
            )  # (num_patches, box_embed_dim)

            quantized_patch_embeddings, _, patch_commitment_loss = self.vq(
                object_patch_embeddings
            )  # (num_patches, box_embed_dim)
            quantized_positional_embeddings, _, positional_commitment_loss = self.vq(
                object_positional_embeddings
            )  # (num_patches, box_embed_dim)
            quantized_object_region_embedding, _, region_commitment_loss = self.vq(
                object_region_embedding
            )  # (1, box_embed_dim)
            parent_embeddings, parent_commitment_loss = self.compute_parent_embeddings(
                quantized_patch_embeddings, quantized_positional_embeddings
            )  # (num_patches, num_patches, box_embed_dim)

            # TODO: Move this loss computation below
            foundational_embeddings_separation_loss = self.compute_foundational_embeddings_loss(
                quantized_patch_embeddings, quantized_positional_embeddings
            )

            parent_ground_truth_boxes = self.compute_pairwise_intersection_boxes(
                quantized_patch_embeddings, quantized_positional_embeddings
            )  # (num_patches, num_patches, box_embed_dim)
            ...
            overall_intersection_box = self.compute_overall_intersection_box(
                torch.cat([quantized_positional_embeddings, quantized_patch_embeddings], dim=0)
            )  # (1, box_embed_dim)

            permutation_set_indices = self.compute_permutation_indices(
                num_patches
            )  # (num_permutations_sets, num_patches, 2)
            num_permutation_sets = permutation_set_indices.shape[0]
            sampled_parent_permutation_sets = parent_embeddings[
                permutation_set_indices[..., 0], permutation_set_indices[..., 1]
            ]  # (num_permutations_sets, num_patches, box_embed_dim)

            sampled_permutation_intersection_boxes = []
            for permutation_idx in range(num_permutation_sets):
                permutation_set = sampled_parent_permutation_sets[
                    permutation_idx
                ]  # (num_patches, box_embed_dim)
                permutation_intersection_box = self.compute_overall_intersection_box(
                    permutation_set
                )  # (1, box_embed_dim)
                sampled_permutation_intersection_boxes.append(permutation_intersection_box)

            sampled_permutation_intersection_boxes = torch.cat(
                sampled_permutation_intersection_boxes, dim=0
            )  # (num_permutation_sets, box_embed_dim)
            ...
            parent_permutation_pairwise_intersection_boxes = (
                self.compute_pairwise_intersection_boxes(
                    sampled_permutation_intersection_boxes,
                    sampled_permutation_intersection_boxes,
                )
            )  # (num_permutation_sets, num_permutation_sets, box_embed_dim)
            parent_permutation_pairwise_volumes = self.compute_box_volume(
                parent_permutation_pairwise_intersection_boxes.reshape(-1, self.box_embed_dim)
            )  # (num_permutation_sets * num_permutation_sets,)
            parent_permutation_pairwise_volumes = parent_permutation_pairwise_volumes.reshape(
                num_permutation_sets, num_permutation_sets
            )  # (num_permutation_sets, num_permutation_sets)
            parent_permutation_pairwise_volumes_upper = parent_permutation_pairwise_volumes.triu(
                diagonal=1
            )
            parent_permutation_separation_loss = torch.sum(
                parent_permutation_pairwise_volumes_upper
            ) / (
                num_permutation_sets * (num_permutation_sets - 1) + 1e-6
            )  # scalar

            correct_parent_set = torch.diagonal(parent_embeddings, 0).permute(
                1, 0
            )  # (num_patches, box_embed_dim)
            correct_parent_intersection_box = self.compute_overall_intersection_box(
                correct_parent_set
            )  # (1, box_embed_dim)
            object_region_alignment_loss = F.mse_loss(
                quantized_object_region_embedding, correct_parent_intersection_box
            )  # scalar
            ...
            batch_loss_tensors[
                "foundational_embeddings_separation_loss"
            ] += foundational_embeddings_separation_loss
            batch_loss_tensors["parent_inclusion_loss"] += parent_inclusion_loss
            batch_loss_tensors[
                "parent_permutation_inclusion_loss"
            ] += parent_permutation_inclusion_loss
            batch_loss_tensors[
                "parent_permutation_separation_loss"
            ] += parent_permutation_separation_loss
    ...
    def training_step(self, batch, batch_idx):
        ...
        batch_loss_tensors = {
            key: value * self.loss_weights[key] for key, value in batch_loss_tensors.items()
        }
    ...
    def configure_optimizers(self):
        ...
        if self.config["trainer"]["optimizer"]["type"] == "Adam":
            return torch.optim.Adam(
                self.parameters(), **self.config["trainer"]["optimizer"]["config"]
            )
        else:
            raise ValueError(
    ...
    def compute_bounding_boxes(self, mask_patches):
        ...
        y_coords = (
            torch.arange(grid_h, device=mask_patches.device)
            .reshape(1, grid_h, 1)
            .expand(B, grid_h, grid_w)
        )  # (B, grid_h, grid_w)
        x_coords = (
            torch.arange(grid_w, device=mask_patches.device)
            .reshape(1, 1, grid_w)
            .expand(B, grid_h, grid_w)
        )  # (B, grid_h, grid_w)
    ...
    def stitch_object_patches(self, object_patches):
        ...
        stitched_image = object_patches.permute(
            2, 0, 3, 1, 4
        )  # (C, bbox_grid_h, patch_h, bbox_grid_w, patch_w)
        stitched_image = stitched_image.reshape(
            C, stitched_h, stitched_w
        )  # (C, stitched_h, stitched_w)
    ...
    def compute_positional_data(self, object_patches):
        ...
        object_grid_coords_y = torch.arange(
            bbox_grid_h, device=object_patches.device, dtype=torch.float32
        )
        object_grid_coords_x = torch.arange(
            bbox_grid_w, device=object_patches.device, dtype=torch.float32
        )
        object_grid_coords_y, object_grid_coords_x = torch.meshgrid(
            object_grid_coords_y, object_grid_coords_x, indexing="ij"
        )  # (bbox_grid_h, bbox_grid_w)
        object_grid_coords = torch.stack(
            [object_grid_coords_y, object_grid_coords_x], dim=-1
        )  # (bbox_grid_h, bbox_grid_w, 2)
        object_grid_coords_normalized = object_grid_coords / torch.tensor(
            [bbox_grid_h - 1, bbox_grid_w - 1], device=object_patches.device
        )  # (bbox_grid_h, bbox_grid_w, 2)
        object_grid_coords_normalized = object_grid_coords_normalized.reshape(
            bbox_grid_h * bbox_grid_w, 2
        )  # (bbox_grid_h * bbox_grid_w, 2)
    ...
    def compute_pairwise_intersection_boxes(self, box_embeddings_1, box_embeddings_2):
        boxes_min_1, boxes_max_1 = self.split_box_embeddings(
            box_embeddings_1
        )  # (N, embed_dim), (N, embed_dim)
        boxes_min_2, boxes_max_2 = self.split_box_embeddings(
            box_embeddings_2
        )  # (N, embed_dim), (N, embed_dim)
        ...
        intersection_boxes = torch.cat(
            [intersection_min, intersection_max], dim=-1
        )  # (N, N, box_embed_dim)
    ...
    def compute_overall_intersection_box(self, box_embeddings):
        boxes_min, boxes_max = self.split_box_embeddings(
            box_embeddings
        )  # (N, embed_dim), (N, embed_dim)
    ...
    def compute_permutation_indices(self, num_elements):
        ...
        cols = torch.tensor(
            permutation_sets, dtype=torch.long
        )  # (num_permutations_sets, num_elements)
        rows = (
            torch.arange(num_elements, dtype=torch.long)
            .unsqueeze(0)
            .expand(num_permutations_sets, -1)
        )  # (num_permutations_sets, num_elements)
    ...
    def compute_foundational_embeddings_loss(
        self, quantized_patch_embeddings, quantized_positional_embeddings
    ):
        N, M = (
            quantized_positional_embeddings.shape[0],
            quantized_patch_embeddings.shape[0],
        )
        foundational_embeddings = torch.cat(
            [quantized_positional_embeddings, quantized_patch_embeddings], dim=0
        )  # (N + num_unique, box_embed_dim)
        foundational_embeddings_min, foundational_embeddings_max = self.split_box_embeddings(
            foundational_embeddings
        )  # (N + num_unique, embed_dim), (N + num_unique, embed_dim)
        foundational_embeddings_centers = (
            foundational_embeddings_min + foundational_embeddings_max
        ) / 2.0  # (N + num_unique, embed_dim)

        foundational_embeddings_distance_margin = self.foundational_embeddings_distance_margin
        foundational_embeddings_pairwise_distances = torch.cdist(
            foundational_embeddings_centers, foundational_embeddings_centers, p=2
        )  # (N + num_unique, N + num_unique)
        foundational_embeddings_separation_loss = F.relu(
            foundational_embeddings_distance_margin - foundational_embeddings_pairwise_distances
        )  # (N + num_unique, N + num_unique)
        ...
        foundational_embeddings_separation_loss = foundational_embeddings_separation_loss.sum() / (
            mask.sum() + 1e-6
        )

        return foundational_embeddings_separation_loss

    def compute_parent_embeddings(
        self, quantized_patch_embeddings, quantized_positional_embeddings
    ):
        num_patches = quantized_patch_embeddings.shape[0]
        pos_expand = quantized_positional_embeddings.unsqueeze(1).expand(
            -1, num_patches, -1
        )  # (N, N, box_embed_dim)
        patch_expand = quantized_patch_embeddings.unsqueeze(0).expand(
            num_patches, -1, -1
        )  # (N, N, box_embed_dim)
        dense_concat = torch.cat([pos_expand, patch_expand], dim=-1)  # (N, N, box_embed_dim * 2)
        parent_embeddings = self.binding_function(
            dense_concat.reshape(num_patches * num_patches, -1)
        )  # (N * N, box_embed_dim)
        parent_embeddings, _, parent_commitment_loss = self.vq(
            parent_embeddings
        )  # (N * N, box_embed_dim)
        parent_embeddings = parent_embeddings.reshape(
